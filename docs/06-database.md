...
迁移在 `internal/serverstore/migrations/`(0001–0017,**0007 已废弃**,文件不存在)。共 17 张表。
...
| is_admin | 0/1 |
| status | 1=启用 |
| quota_tokens | 0017 新增,月流量配额三态:NULL=跟随全局默认(`usage.monthly_quota`),0=不限,>0=按月限额;admin 一律豁免(网关强制) |
