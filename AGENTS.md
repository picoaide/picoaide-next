...
4. **高危操作必须人确认**——删除/截屏/剪贴板读/命令/浏览器操作/kb_upload 等 → 审批门控(60s 超时拒绝)。**实现裁决(2026-08-12)**:审批经 AI SDK v7 原生 `toolApproval` 实现(引擎主循环内逐个 `confirm_required` → 等回执 → `tool-approval-response`),不再自管 execute 层门控;`ai` 版本锁定于 package.json,升级 SDK 必须跑 `engine.test.ts` 审批用例(串行审批/超时自动拒绝/迟到回执 no-op);审批弹窗展示实际命令串/代码/URL(与执行串一致,`confirm_required.target`),拒绝盲批。
...
7. **商城与知识库均为管理员授权制(严格默认)**——资源(知识库文件夹/技能/MCP)上架/创建后**未授权用户一律不可见不可用**(404 不泄露存在性);授权对象 = 用户或部门组(@组名约定,组名大小写不敏感);admin 恒全量不落表;授权变更必审计(kb_audit_logs);改密/降权/禁用自动吊销全部 API token。
8. **部门(组)即金字塔组织架构**(迁移 0017)——`groups` 含 parent_id/leader_id:部门树任意层级、部门主管、员工**单部门归属**(`PUT /api/admin/users/:id/department`);权限继承:`UserEffectiveGroups` = 归属部门+祖先链(授权给部门覆盖子部门成员)+ 主管部门子树(主管向上继承)+ 隐式「全员」组(全员为保留名,禁建/删/改名);部门改名级联授权表(NOCASE)、删除须无成员/子部门/授权引用;LDAP 登录全量同步组(空组即回收)。
8. **浏览器操作 = 员工自己的浏览器**——客户端主进程固定监听 `127.0.0.1:54321`,Chrome/Edge 插件默认直连即装即用;纯本地 WebSocket 通道,不经服务端、离线可用。**信任边界(2026-08-12 裁决)**:桥仅绑定回环地址,信任模型 = 本机进程即员工本人;任意本机进程可经桥驱动浏览器属设计内行为(非安全边界),后续可做 capability token 升级。
...
```
renderer(React+shadcn) ──preload(contextBridge)──▶ main(Node)
                                                     ├─ Agent 引擎:AI SDK streamText 多步循环 + 审批门控
                                                     ├─ 本地工具:文件/终端/沙盒/web/屏幕/OCR/剪贴板
                                                     ├─ 浏览器插件桥:CDP 服务 127.0.0.1:54321(插件直连)
                                                     ├─ 本地 MCP 运行时 + Skill 运行时
                                                     ├─ better-sqlite3(会话/消息/产物/设置)
...
desktop/               # Electron 客户端
  src/main/            #   index/ipc/cdp_server + agent/ + tools/ + mcp/ + skill/ + store/ + gateway/(auth/bootstrap/tls/health/remote_mcp/marketplace)
  src/preload/         #   contextBridge 白名单 API
  src/renderer/        #   api/ + components/(ui=shadcn + 业务组合) + pages/(Login/Main/Settings) + stores/(Zustand)
